    ...
    void token_Insert(const std::string &text, std::vector<lex::Token> &tokens) {
        if(tokens.size()>=3) {
            if(tokens[1].getTokenType() != lex::TOKEN_HEX) {
                stream << "Error: start address must be hex value.<br>";
                return;
            }
            ...
            for(i = 2; i < tokens.size(); ++i) {
                if(tokens[i].getTokenType() == lex::TOKEN_CHAR) {
                    if(!code.symbols.exisits(tokens[i].getToken())) {
                        stream << "Error: Symbol: " << tokens[i].getToken() << " not found in table..<br>";
                        return;
                    }
                }
                else if(tokens[i].getTokenType() != lex::TOKEN_HEX) {
                    stream << "Error: list of values must be either symbol or hex value.<br>";
                    return;
                }
            }
            for(i = 2; i < tokens.size(); ++i) {
                ...
                if(address > 0xFFFF) {
                    stream << "Error: memory out of bounds..<br>";
                    return;
                }
                uint8_t hexValue = 0;
                if(tokens[i].getTokenType() == lex::TOKEN_CHAR)
                ...
                stream << "[ $" << std::setfill('0') << std::setw(4) << std::hex << std::uppercase << address << " ] = $" << std::setfill('0') << std::setw(2) << std::hex << std::uppercase << static_cast<uint32_t>(hexValue) << "<br>";
                
                code.poke(static_cast<uint16_t>(address),hexValue);
            }
        } else {
            stream << "Error: requires starting address followed by values<br>";
            return;
        }
    }
}
